...
.. image:: http://0.0.0.0:8000/png/exprogressindicator-types.png
    :align: center
...
.. image:: http://0.0.0.0:8000/png/exprogressindicator-linear-usage.png
    :align: center
...
.. image:: http://0.0.0.0:8000/png/exprogressindicator-circular-usage.gif
    :align: center
...
You can animate determinate progress using :class:`kivy.animation.Animation`
with the ``easing_emphasized`` transition.
...
.. image:: http://0.0.0.0:8000/png/exprogressindicator-determinate.gif
    :align: center
    :alt: Determinate expressive progress indicator animation.

.. image:: http://0.0.0.0:8000/png/exprogressindicator-indeterminate.gif
    :align: center
...
.. image:: http://0.0.0.0:8000/gif/exprogressindicator-linear-contiguous.gif
    :align: center
...
.. image:: http://0.0.0.0:8000/gif/exprogressindicator-linear-discontinuous.gif
    :align: center
...
.. image:: http://0.0.0.0:8000/gif/exprogressindicator-circular-advanced.gif
    :align: center
...
.. image:: http://0.0.0.0:8000/gif/exprogressindicator-circular-retreat.gif
    :align: center
...
class MDExBaseProgressBar(Widget, DeclarativeBehavior, ThemableBehavior):
    ...
    wave_length = NumericProperty(dp(20))
    ...
    def __init__(self, **kwargs):
        ...
        Clock.schedule_once(lambda t: self.on_determinate(self, True))
    ...
    def reset_colors(self):
        ...
        for line in self._active_names:
            self.color_obj(line).rgba = self.theme_cls.primaryColor
        # inactive lines
        for line in self._inactive_names:
            self.color_obj(line).rgba = (
    ...
    def setup_lines(self):
        """init line objs"""
        active_groups = [
            self.canvas.get_group(name) for name in self._active_names
        ]
        inactive_groups = [
            self.canvas.get_group(name) for name in self._inactive_names
        ]
        if not all(active_groups) or not all(inactive_groups):
            return
        self._active_line_objs = [group[0] for group in active_groups]
        self._inactive_line_objs = [group[0] for group in inactive_groups]
...
class MDExLinearProgressIndicator(MDExBaseProgressBar):
    ...
    def on_determinate(self, instance, value):
        if not self.canvas or not self.canvas.get_group("dot"):
            Clock.schedule_once(lambda dt: self.on_determinate(instance, value))
            return
        ...
        # reset colors
        if self.determinate or self.indeterminate_animator == "discontinuous":
            self.reset_colors()
...
class MDExCircularProgressIndicator(MDExBaseProgressBar):
    ...
    def save_frame_context(self, *args):
        ...
        # spacing in degree
        spacing = (self.spacing + thickness) / radius
    ...
    def get_start_and_end(self, init_rad, s_f, e_f):
        return init_rad + (s_f * 2 * math.pi), init_rad + (e_f * 2 * math.pi)
